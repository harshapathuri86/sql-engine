...
def get_relate_op(cond):
    if "<=" in cond:
        ...
        errorif(True, "invalid condition : '{}'".format(cond))
...
def get_output_table(qdict):
    ...
    inter_table = [[i for tup in r for i in list(
        tup)] for r in itertools.product(*all_tables)]
        ...
        for idx, (op, left, right) in enumerate(conditions):
            ...
            for tname, cname in [left, right]:
                if tname == LITERAL:
                    ...
                    cols.append(inter_table[:, colidx[tname][cname]])
    ...
    if proj_cols[0][2]:
        ...
        for idx, (tn, cn, aggr) in enumerate(proj_cols):
            ...
            if aggr == "min":
                ...
                seen = set()
                out_table = [x for x in col.tolist() if not (
                    x in seen or seen.add(x))]
                disti = True
            else:
                errorif(True, "invalid aggregate")
        out_table = np.array([out_table])
        if disti:
            out_table = np.array(out_table).T
        out_header = ["{}({}.{})".format(aggr, tn, cn)
                      for tn, cn, aggr in proj_cols]
    else:
...
def break_query(q):
    ...
    errorif((len(select_idx) != 1) or (len(from_idx) != 1)
            or (len(where_idx) > 1), "invalid query")
    ...
    if where_idx:
        errorif(where_idx <= from_idx, "invalid query")
    ...
    errorif(where_idx != None and len(raw_condition)
            == 0, "no conditions after 'where'")
    # ----------------------------------------------
...
def parse_tables(raw_tables):
    ...
    for rt in raw_tables:
        t = rt.split()
        errorif(not(len(t) == 1 or (
            len(t) == 3 and t[1] == "as")), "invalid table spacification '{}'".format(rt))
        if len(t) == 1:
            tb_name, tb_alias = t[0], t[0]
        else:
            tb_name, _, tb_alias = t

        errorif(tb_name not in schema.keys(),
                "no table name '{}'".format(tb_name))
        errorif(tb_alias in alias2tb.keys(),
                "not unique table/alias '{}'".format(tb_alias))
...
def parse_proj_cols(raw_cols, tables, alias2tb):
    ...
    for rc in raw_cols:
        ...
        errorif("." in rc and len(rc.split(".")) != 2,
                "invalid column name '{}'".format(rc))

        # get table name and column name
        tname = None
        if "." in rc:
            tname, cname = rc.split(".")
            errorif(tname not in alias2tb.keys(),
        ...
        if cname == "*":
            ...
            if tname != None:
                proj_cols.extend([(tname, c, aggr)
                                 for c in schema[alias2tb[tname]]])
            else:
                for t in tables:
                    proj_cols.extend([(t, c, aggr)
                                     for c in schema[alias2tb[t]]])
        else:
            errorif(cname not in schema[alias2tb[tname]],
                    "unknown field : '{}'".format(rc))
    ...
    errorif(all(s) ^ any(
        s), "aggregated and nonaggregated columns are not allowed simultaneously")
    errorif(any([(a == "distinct") for a in s]) and len(
        s) != 1, "distinct can only be used alone")
    # ----------------------------------------------
    return proj_cols
